...
// Same reasoning as postings/db.ts's own getPool (see its comment) — entitlement/trial-usage
// checks run on the live per-message path (approving a match), so an untimed stalled query here
// would silently hang a customer's whole conversation rather than fail loudly.
