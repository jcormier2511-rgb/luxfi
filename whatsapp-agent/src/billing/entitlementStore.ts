 ...
 * further approvals right now is an admin manually flipping `manual_override_enabled` —
 * never a live charge, never self-service. `membershipVerified`/`paymentAuthorized`/
 * `paymentStatus` are placeholder columns: unused by any gating logic today, but present so
 * wiring in real WatchFacts membership verification or a payment processor later is an
 * UPDATE to existing rows, not another schema migration.
 */
