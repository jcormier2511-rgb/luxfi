...
// Same reasoning as postings/db.ts's own getPool (see its comment) — this pool backs the live
// v3 search-on-demand query path (findMatches/getActiveListings), so an untimed stalled query
// here would silently hang a customer's whole conversation, not just fail loudly.
