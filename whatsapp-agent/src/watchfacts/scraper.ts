 ...
 *
 * Unlike the Trading Floor feed (see src/watchfacts/api.ts), no JSON API has been found for
 * a single profile's listings page, so this one contact-intro-personalization feature still
 * relies on DOM scraping. Validate with `npm run wf:test -- <profileId>` before trusting it.
 */
