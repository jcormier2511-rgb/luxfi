...
async function preferredIdentity(user:number):Promise<string|null>{return withSchema(async db=>(await db.query(`SELECT li.identity FROM linked_identities li LEFT JOIN canonical_notification_preferences p ON p.canonical_user_id=li.canonical_user_id WHERE li.canonical_user_id=$1 ORDER BY (li.id=p.preferred_identity_id) DESC,li.id LIMIT 1`,[user])).rows[0]?.identity??null)}
