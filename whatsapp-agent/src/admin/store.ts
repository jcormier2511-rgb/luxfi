...
const publicAdmin = (r:any): Administrator => ({ id:r.id, name:r.name, username:r.username, email:r.email, role:r.role, status:r.status, last_login_at:r.last_login_at?.toISOString?.() ?? r.last_login_at, created_at:r.created_at?.toISOString?.() ?? r.created_at, updated_at:r.updated_at?.toISOString?.() ?? r.updated_at });
