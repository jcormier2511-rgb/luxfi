...
const GROUP_FORM = `<section class="card" id="group-form-card">
  ...
  <p class="muted">The group ID is platform-specific: a WhatsApp group's digits, or a Telegram group/supergroup's numeric chat id (negative, e.g. -1001234567890) — grab it from the server logs after Fi is added and someone posts, or GET /admin/group-listings. Wildcards are never accepted. Entering a group ID that already exists loads and updates that record instead of creating a duplicate — platform + group ID together are what must be unique.</p>
  <div class="toolbar">
    ...
    <div class="field"><label for="gf-groupid">Group ID</label><input id="gf-groupid" placeholder="Group chat id"></div>
    <div class="field"><label for="gf-source-account">Source account (optional)</label><input id="gf-source-account" placeholder="Which connected account found this group"></div>
  </div>
  <div class="toolbar">
  ...
  <div class="toolbar">
    <label class="inline"><input type="checkbox" id="gf-push-enabled"> Push enabled</label>
    <label class="inline"><input type="checkbox" id="gf-push-fs" checked> Allow push FS</label>
    <label class="inline"><input type="checkbox" id="gf-push-wtb" checked> Allow push WTB</label>
    <div class="field"><label for="gf-priority">Priority</label><input id="gf-priority" type="number" value="100" placeholder="Lower posts first"></div>
    <div class="field"><label for="gf-category">Category (optional)</label><input id="gf-category" placeholder="e.g. luxury-watches"></div>
  </div>
  <div class="toolbar">
    <div class="field"><label for="gf-fi-member">Fi is a member</label><select id="gf-fi-member"><option value="">Unknown</option><option value="true">Yes</option><option value="false">No</option></select></div>
  ...
  <pre id="gf-health" class="muted"></pre>
</section>`;
...
export function renderManagementPage(kind:"users"|"groups"|"administrators"|"coverage"):string {
  ...
  ${kind==='groups'?`
  let gfEditId=null;
  function groupReset(){gfEditId=null;document.querySelector('#group-form-title').textContent='Add group';document.querySelector('#gf-name').value='';document.querySelector('#gf-platform').value='whatsapp';document.querySelector('#gf-groupid').value='';document.querySelector('#gf-source-account').value='';document.querySelector('#gf-status').value='active';document.querySelector('#gf-monitoring').checked=true;document.querySelector('#gf-fs').checked=true;document.querySelector('#gf-wtb').checked=true;document.querySelector('#gf-push-enabled').checked=false;document.querySelector('#gf-push-fs').checked=true;document.querySelector('#gf-push-wtb').checked=true;document.querySelector('#gf-priority').value='100';document.querySelector('#gf-category').value='';document.querySelector('#gf-fi-member').value='';document.querySelector('#gf-country').value='';document.querySelector('#gf-notes').value='';document.querySelector('#gf-error').textContent='';document.querySelector('#gf-health').textContent=''}
  function groupEdit(id){const r=lastRows.find(x=>x.id===id);if(!r)return;gfEditId=id;document.querySelector('#group-form-title').textContent='Edit group #'+id;document.querySelector('#gf-name').value=r.group_name||'';document.querySelector('#gf-platform').value=r.platform||'whatsapp';document.querySelector('#gf-groupid').value=r.group_id||'';document.querySelector('#gf-source-account').value=r.source_account||'';document.querySelector('#gf-status').value=r.status||'active';document.querySelector('#gf-monitoring').checked=!!r.monitoring_enabled;document.querySelector('#gf-fs').checked=r.monitor_fs!==false;document.querySelector('#gf-wtb').checked=r.monitor_wtb!==false;document.querySelector('#gf-push-enabled').checked=!!r.push_enabled;document.querySelector('#gf-push-fs').checked=r.allow_fs!==false;document.querySelector('#gf-push-wtb').checked=r.allow_wtb!==false;document.querySelector('#gf-priority').value=r.priority??100;document.querySelector('#gf-category').value=r.category||'';document.querySelector('#gf-fi-member').value=r.fi_is_member===true?'true':r.fi_is_member===false?'false':'';document.querySelector('#gf-country').value=r.country||'';document.querySelector('#gf-notes').value=r.notes||'';document.querySelector('#gf-error').textContent='';document.querySelector('#gf-health').textContent='Last message seen: '+(r.last_message_at||'never')+' | Last ingestion: '+(r.last_successful_ingest_at||'never')+' | Last push: '+(r.last_push_at||'never')+(r.last_push_result?' ('+r.last_push_result+')':'')+' | Accessible: '+(r.accessible===false?'no':'yes')+(r.last_verified_at?' (verified '+r.last_verified_at+')':'');window.scrollTo(0,0)}
  async function groupDelete(id){if(!confirm('Delete group #'+id+'? This cannot be undone.'))return;try{const token=await ensureCsrf();const res=await fetch('/admin/api/groups/'+id+'?confirm=true',{method:'DELETE',headers:{'X-CSRF-Token':token}});if(res.status===403){location.href='/admin';return}if(!res.ok){const data=await res.json().catch(()=>({}));document.querySelector('#error').textContent=data.error||'Delete failed';return}load()}catch(e){document.querySelector('#error').textContent=e.message}}
  async function groupSave(){
    document.querySelector('#gf-error').textContent='';
    const fiMemberRaw=document.querySelector('#gf-fi-member').value;
    const body={group_name:document.querySelector('#gf-name').value.trim(),platform:document.querySelector('#gf-platform').value,group_id:document.querySelector('#gf-groupid').value.trim(),source_account:document.querySelector('#gf-source-account').value.trim(),status:document.querySelector('#gf-status').value,monitoring_enabled:document.querySelector('#gf-monitoring').checked,monitor_fs:document.querySelector('#gf-fs').checked,monitor_wtb:document.querySelector('#gf-wtb').checked,push_enabled:document.querySelector('#gf-push-enabled').checked,allow_fs:document.querySelector('#gf-push-fs').checked,allow_wtb:document.querySelector('#gf-push-wtb').checked,priority:Number(document.querySelector('#gf-priority').value)||100,category:document.querySelector('#gf-category').value.trim(),fi_is_member:fiMemberRaw===''?null:fiMemberRaw==='true',country:document.querySelector('#gf-country').value.trim(),notes:document.querySelector('#gf-notes').value.trim()};
    if(!body.group_name||!body.group_id){document.querySelector('#gf-error').textContent='Group name and group ID are required';return}
    try{
  ...
  async function groupSyncGreenApi(){
    const result=document.querySelector('#grp-bulk-result');
    result.textContent='Syncing…';
    try{
      const token=await ensureCsrf();
      const res=await fetch('/admin/api/groups/sync-greenapi',{method:'POST',headers:{'X-CSRF-Token':token}});
      if(res.status===403){location.href='/admin';return}
      const data=await res.json();
      if(!res.ok){result.textContent=data.error||'Sync failed';return}
      result.textContent='Discovered '+data.discovered+', created '+data.created+', updated '+data.updated+', marked inaccessible '+data.markedInaccessible+'.';
...
function renderMarketUpdatesCard(mu: AdminDashboardData["marketUpdates"]): string {
  ...
  return `<section class="card">
    ...
    <dl>
      <dt>Schedule</dt><dd>${escapeHtml(mu.dayOfWeek)}s at ${escapeHtml(mu.time)}</dd>
