...
const review=(type:string,p:PendingSellIntake|PendingBuyIntake,price:number)=>[`${type} listing review`,`Brand: ${displayBrand(p.brand)||"Not provided"}`,`Model: ${p.model||("modelSkipped" in p && p.modelSkipped ? "Any" : "Not provided")}`,`Reference: ${p.reference||"Not provided"}`,p.dialColor&&`Dial: ${p.dialColor}`,p.condition&&`Condition: ${cap(p.condition)}`,`${type==="FS"?"Price":"Budget"}: ${cash(price,p.currency)}`,p.location&&`Location: ${cap(p.location)}`,p.boxPapers&&`Box/Papers: ${p.boxPapers}`,p.year&&`Year: ${p.year}`,`Photo: ${"imageUrl" in p&&p.imageUrl?"attached":"none"}`,`Reply "CONFIRM" to start monitoring, or send a correction.`].filter(Boolean).join("\n");
